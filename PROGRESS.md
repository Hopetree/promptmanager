...
## 阶段 56（2026-09-25）：关于页改为开源项目形态（FR-124 / AC-120）

> 环境：按**已确认的终版口径**用**开发环境**（`DATA_DIR=./dev-data` + 端口 8765，复用既有 5 prompts / 3 folders / 2 tokens，**未重造夹具**）；
> **8767 全程未碰**（host_manger 的测试环境）。资源纪律：串行单上下文、每项查一次资源、熔断未触发（avail 2104–2340MB、load1 ≤6.1，阈值 800MB/20）。

### 1. 改了什么

| 分区 | 内容 | 落点 |
| --- | --- | --- |
| ① 身份区 | 一句话定位（`PM_META.description`）+ 版本徽标/在线状态/重新探测（**照旧**）+ **访问地址（从服务区移来）** + 8 个关键词标签 | `AboutModal.tsx` `pm-about-identity` |
| ② 出处与去向区（新增） | 代码仓库 / Docker 镜像（可复制）/ 文档说明 / 问题反馈 / 许可证 **五项** | `AboutModal.tsx` `pm-about-links` |
| ③ 使用 / 维护区 | 保留；**移动端默认折叠** | `AboutModal.tsx` |
| ~~服务区~~ | **整个去掉**（含那句错的「拷贝 pm.db」），按 D-56 ④ **不改写成别的样子** | — |

**元信息单一来源**（D-56 ①）：`vite.config.ts` 读 `package.json` → 规范化（剥 `git+`、剥 `.git`、镜像名小写反推）→ `define` 内联 `__PM_META__` → 前端只经 `web/src/pm-meta.ts` 读。
组件里**不再有任何** `github.com` / `Hopetree` / `hopetree` / `MIT` 字面量（测试逐个断言）。
...
```
页面: 轻量 Prompt 管理器：单进程自托管（Node + SQLite + React/antd）
README 首句: 轻量、自托管、数据自持的 Prompt 管理器。一个进程、一个端口、一个数据库文件，浏览器打开就能用。
```
两处口径一致（都点明"轻量 + 自托管"）。值取自 `package.json.description`（**非硬编码**，改仓库即同步）。

**② 版本徽标与 /healthz 一致**

```
页面: "PromptManager 后端在线 版本 1.3.0 重新探测"
$ curl -s http://127.0.0.1:8765/healthz → {"status":"ok","version":"1.3.0"}
```
**1.3.0 = 1.3.0** ✅（运行时动态，未从 package.json 注入 version）。

**③ 关键词标签**：`prompt, prompt-management, self-hosted, sqlite, react, antd, llm, mcp` —— 与 `package.json` 的 `keywords` 及实际依赖一致。

**③b 访问地址在身份区且随协议**：`pm-about-address` 实测 `"访问地址 http://127.0.0.1:8765"`，取 `window.location.origin`；断言钉住"不写死 `http://` / `https://`"（阶段 53 的 R-9 修复未回归、未被删）。

**③c 无版本检查 / 无出网请求**：`hasUpdateCheck=false`（全文无"检查更新/新版本/releases/latest"），**`outboundCount=0`**（拦下所有非同源请求，零出网）。依赖表也无任何更新检查库。

**④ 五项渲染文本**

| 项 | 渲染文本 |
| --- | --- |
| 代码仓库 | `github.com/Hopetree/promptmanager` |
| Docker 镜像 | `hopetree/promptmanager`（带复制图标）+「拉取时可用 `:latest` 或某个具体版本标签」 |
| 文档说明 | `README · 使用与部署` |
| 问题反馈 | `GitHub Issues` |
| 许可证 | `MIT` |
...
```
hrefCount = 4   allHaveTarget = true   allHaveNoopener = true
<a href="https://github.com/Hopetree/promptmanager" target="_blank" rel="noopener noreferrer" …>
<a href="https://github.com/Hopetree/promptmanager#readme" target="_blank" rel="noopener noreferrer" …>
<a href="https://github.com/Hopetree/promptmanager/issues" target="_blank" rel="noopener noreferrer" …>
<a href="https://github.com/Hopetree/promptmanager/blob/main/LICENSE" target="_blank" rel="noopener noreferrer" …>
```
（原为 0；4 个链接 + 1 个可复制镜像 = 五项。`target`/`rel` 统一写在 `ExtLink` 一处，不会漏。）

**⑥⑦⑧ 规范化 / 镜像 / 许可证**

```
package.json repository = git+https://github.com/Hopetree/promptmanager.git   ← 需规范化
页面 repoUrl              = https://github.com/Hopetree/promptmanager          ← owner/repo 规范形式
镜像  = hopetree/promptmanager（由仓库地址反推小写）= README 同名 = CI docker.yml 产出规则
提示用 :latest，**无任何写死历史版本号**（断言 /hopetree\/promptmanager:\d/ 不存在）
package.json license = MIT ；LICENSE 首行 = "MIT License"   ✅ 一致
```

**⑨ 维护区命令逐条核对**（5 条全部核对存在性，均仍成立）

| 命令 | 核对结果 |
| --- | --- |
| `node bin/pm.mjs user set-password --username admin` | ✅ `bin/pm.mjs` 在；子命令见 `src/server/cli.ts:214` |
| `journalctl -u promptmanager -f` | ✅ 单元名 = `deploy/promptmanager.service`（`promptmanager.service`） |
| `sudo systemctl restart promptmanager` | ✅ 同上 |
| `git checkout <提交> && npm run build && sudo systemctl restart promptmanager` | ✅ `npm run build` 在 `package.json:11` |
| `docs/dependencies.md` | ✅ 文件在（24,854 B） |

**使用区 7 条**抽核 3 条与真实交互一致：顶栏 `header-new` 存在、「复制提示词」入口存在、`/` 聚焦搜索与 `Esc` 关详情在 `Workspace.tsx:582/561`。

**⑭⑮ 移动端 / PC**

```
移动 440×956: 折叠态 modalH = 611 < 956（不用滚）  initiallyOpen = false ✅
             展开「使用」→ 7 条；展开「维护」→ 5 条命令；展开后 modalBottom = 956 = innerH（无裁切）
             pageOverflowX = 0
PC 1440×900:  pageOverflowX = 0；维护区展开后 wrap 可滚
```

**⑯ 范围克制**：`git diff --stat` 仅 5 个改动文件 + 3 个新增（关于页 1 + 配置 1 + 元信息 1 + 测试 3 + 探针 1），
**未动 package.json / LICENSE / README.md**（按 D-56 ⑦ 只读不改）、未新增迁移、未改接口契约。

**⑰ 回归**：`npm test` **476/476**、`rm -rf dist && ci-check` **6/6**。

### 3. 更新了 3 条既有断言（未删任何）

| 文件 | 原断言 | 更新为 |
| --- | --- | --- |
| `tests/navigation-hygiene.test.ts`（AC-52） | 钉死三分区含 `key: 'service'` | 服务区按 FR-124 取消 → 只钉 usage/maintain，并断言 `service` **不得复活** + 三个新锚点 |
| `tests/stage53-about-origin.test.ts`（A①/A④） | A④ 钉死"其它信息一个字没变"（含数据文件/备份方式两行） | 按 v68 规格更新为"身份区仍完整 + 服务区确已移除 + 不得再教拷贝 pm.db"；品牌/状态条/维护命令/使用 7 条拆成独立测试保留 |
| `tests/stage18-bundle.test.ts` | 预算累计到阶段 55 | 登记 `STAGE56_ACCOUNTED_DELTA = 1260`（420,468 → 421,728 B，实测并用 HEAD 版 AboutModal 复核基线） |

### 4. 新增测试

`tests/stage56-about.test.ts`（11 条）：定位取自 package.json、版本仍运行时、origin 未回归、无版本检查/无出网、
五项齐全 + ExtLink 安全属性、规范化与镜像/许可证、元信息单一来源、服务区已除且含错文案、维护 5 条、使用 7 条、移动端默认折叠。
**红绿对照**：把 `AboutModal.tsx` 换回 HEAD 版本 ⇒ **6/11 变红**，恢复后 11/11 通过。

### 5. 踩坑留痕

1. **查"文案是否还在"必须先剥注释** —— 组件注释里恰好记着"为什么删掉『拷贝 pm.db』"，
   全文 `includes` 会把这段说明当成"文案还在"而误判。测试里统一走 `code()` 去注释后再查。
2. **断言写反比断言写错更隐蔽** —— 我一度写成 `assert.equal(/version:\s*String\(pkg\.version/.test(c), true, '不应出现')`，
