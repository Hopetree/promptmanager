...
| 项 | 值 |
| --- | --- |
| 阶段 | **阶段 1–47 已全部完成**；已发布 **v1.2.0**（阶段 43 已发版） |
...
**阶段 47（FR-110 / AC-111 + FR-111 / AC-112：**版本对比默认「上一版 ↔ 最新」** 〔根因：默认 pair 取的是**升序数组两端** ⇒ 最早↔最新；改为倒数第二个↔最新，单版本退化为 v1↔v1〕+ **令牌页「创建时间」列**〔复用 `formatDateTime` 与「最近使用」同格式；PC 抽屉 640→720 使 7 列无横滚；移动端 7 列全 >0、首列仍是「名称」、两端可达〕，62 条判据全过）自检全过**（见本文件「阶段 47」）；⚠️ 阶段 37 的 FR-98/AC-100（4 列+折叠）已被用户推翻、**自 v49 起作废** |
