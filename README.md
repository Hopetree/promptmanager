...
| **给脚本与 agent 用** | API Token + 使用侧 CLI（`pm get` / `pm render`）+ MCP（只读：**本地 stdio** 或**远程 HTTP**，详见 [接口参考](docs/api.md)） |
| **令牌分权限** | 令牌分**只读**/**读写**两种（新建默认**只读**）：**只读给 MCP / agent 用**（能搜索、查看、渲染），读写才允许新建 / 修改 / 删除；**权限可以在界面里改**（点「状态」列，立即生效）；**令牌管理与改口令只能用界面** |
| **令牌随时可取** | 令牌明文加密保存在本机，**列表里点「复制」就能再取**（不必担心"只显示一次"）；浏览器若拦截自动复制，点「显示」后可手动选中复制 |
| **令牌可清理** | 不再需要的令牌：先「撤销」（立即失效、留痕），再「删除」（永久移除该行） |
...
即 `docker pull hopetree/promptmanager:latest` 就能拉到（`latest` 对应最新发布版本；也可指定具体版本号，如 `:1.3.0`）（在 Docker Hub 上搜 `hopetree/promptmanager` 也能找到）。
下文命令里的 **`<命名空间>` 就是它** —— 你可以把 `<命名空间>/promptmanager` 整个读作 `hopetree/promptmanager`；
照抄时把 `<命名空间>` 替换成 `hopetree` 即可（留着占位符是为了让命令的"哪个部分是命名空间"一目了然）。

```bash
# ① 拉取（<命名空间> 就是 hopetree；把 `<版本>` 换成你要的版本，也可用 1.0 或 latest）
docker pull <命名空间>/promptmanager:<版本>
# 等价写法（把占位符填好，可直接复制）：
docker pull hopetree/promptmanager:<版本>
...
> 若你的镜像站把路径写成了扁平名（如 `<镜像站>/promptmanager:<版本>`），把上面的 `<镜像站>/hopetree` 整段换成它给的名字即可。
...
# ⑥ 自检
curl -s http://127.0.0.1:8767/healthz     # 形如 {"status":"ok","version":"1.3.0"}（与所拉镜像一致）
```
...
| 给脚本发令牌 | `⋯更多 → API 令牌` → 创建；之后随时点该行的「复制」再取明文（或「显示」手动复制） |
| 改令牌权限 | 令牌列表里点「状态」列的权限文字（`有效 · 只读` / `有效 · 读写`）即可在**只读 ↔ 读写**之间切换，**立即生效**（不用重建令牌、不用重新登录）；已撤销的令牌没有这个入口 |
| 手机上管令牌 | 手机上打开 `⋯更多 → API 令牌`：列表**左右滑动**即可看到全部 6 列（右侧的「操作」列能滑到）；创建表单是竖排的，名称输入框独占一行 |
| 清理旧令牌 | 先点「撤销」（立即失效），该行随后会出现「删除」→ 永久移除（不可恢复） |
| 远程接入 MCP | 见下方「远程 MCP 接入」 |

**快捷键**：`/` 聚焦搜索框，`Ctrl/Cmd + K` 同样聚焦搜索，`Esc` 关闭当前浮层/抽屉。

### 远程 MCP 接入（给 AI 客户端 / agent 用）

本服务内置一个 **只读** 的 MCP 端点，AI 客户端（如 QwenPaw）只要填 **URL + 认证头** 就能远程接入，
**不需要在客户端所在机器上放任何副本或启动器**：
...
令牌在界面 `⋯更多 → API 令牌` 里创建（之后也能随时点「复制」再取）。示例（curl 手动试一次）：

```bash
curl -s -X POST http://127.0.0.1:8767/mcp \
  -H 'Authorization: Bearer pm_你的令牌' \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json, text/event-stream' \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}'
```

它提供三个**只读**工具：`prompt_search`（检索）、`prompt_get`（取单条）、`prompt_render`（渲染变量出成品）。
**没有任何写操作**；每次取用都会记入使用记录（归属到你用的那个令牌）。
若前面有反向代理，记得把 `/mcp` 路径也转发过来并**透传 `Authorization` 头**。
...
1. **换国内镜像站 / 加速地址**（见效最快）：先从镜像站拉下来，再 `tag` 回规范名，后续命令不用改：
   ...
   若镜像站给的是扁平名（`<镜像站>/promptmanager:<版本>`），把 `<镜像站>/hopetree` 整段替换成它给的名字。
...
2. **给 Docker 配代理**（能连外网但慢）：在 `/etc/systemd/system/docker.service.d/` 下加
   ...
   之后直接 `docker pull hopetree/promptmanager:<版本>` 即可，**不需要**再 tag。
...
**令牌明文还能再看到吗？**
能。令牌明文以加密形式保存在你的数据目录里（**不是**只显示一次）：在 `⋯更多 → API 令牌` 里点该行的「复制」即可再取
（创建时也不再弹"只显示一次"的窗口，列表里随时能取）。如果浏览器不允许网页自动写剪贴板（内网 `http://` 访问时常见），
点同一行的「显示」会**把明文显示在页面上**，你可以手动选中复制。
加密密钥在数据目录下（`token-enc.key`，仅本人可读），也可以用环境变量 `TOKEN_ENC_KEY` 指定。
注意：**升级前创建的老令牌看不到明文**（当时没有保存），界面会提示"不可查看"，撤销后重建即可 —— 老令牌在此期间照常能用。

**远程 MCP 接入报 401？**
检查请求头是否是 `Authorization: Bearer <令牌>`（注意 `Bearer` 后面有一个空格）、令牌是否被撤销、以及地址是否是
`http://<主机>:8767/mcp`。另外 MCP 端点是**无状态**的：只用 `POST` 发 JSON-RPC，不需要、也不支持建立会话。
...
- **老令牌不可查看明文**：升级前创建的令牌当时没有保存明文，无法恢复（撤销后重建即可；鉴权不受影响）。
- **删除令牌不可恢复**：对已撤销的令牌点「删除」是**真删**（那一行与审计记录一并消失）；只想让它失效请用「撤销」。
- **MCP 端点是无状态的**：不支持长连接会话/服务端推送，只支持请求-响应式的只读工具调用。
